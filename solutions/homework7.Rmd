...
2. For the general case of the chain of length $N$ with $K$ values in each node the complexity is $K^N$, i.e. it grows exponentially in chain length.
...
1. In the lecture it was shown that in general case message passing algorithm implies complexity of $O(NK^2)$, where $N$ is the number of nodes in the chain, and $K$ is the number of possible values of each node.

2. Following the formula above, the computational complexity for $P(X_4=1)$ is $4 \cdot 2^2 = 16$.  
The complexity of computing all marginal probability distributions is also 16, if all the messages are stored.
